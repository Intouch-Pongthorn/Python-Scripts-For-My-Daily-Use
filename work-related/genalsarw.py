...
def main():
    ...
    #fetch only sarxx files from the directory
    sar_files = get_sar_files(path)
    try : 
        #delete all_sar.txt if exists 
        os.path.exists(alsar_location) and os.remove(alsar_location)
        #open all_sar.txt (if there's no  all_sar.txt file, this line creates the file)
        all_sar = open(alsar_location,'a')
        print("Writing all_sar.txt.....")
        #append content from each sarxx file to all_sar.txt line by line
...
def get_sar_files(path:str)->list[str]:
    #pick only sarXX.file into a list
    files_list = [file for file in glob(f"{path}\sar[0-9][0-9]")]
    #sort sarXX.file from the list above by modified date
    files_list.sort(key = lambda file: os.stat(file).st_mtime)
    #raise an error if no sar files found in the directory and ends the program
    if len(files_list) == 0:
        raise FileNotFoundError(os.strerror(errno.ENOENT),f"no sar files found in {path}")
    print("Arranging sar files in order.......")
    #echo file names and time of modification on the terminal screen
